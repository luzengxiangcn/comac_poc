...
async def _generate_tender_task(
    ...
    try:
        ...
        # 从响应列表中提取完整内容
        markdown_content = ""
        if hasattr(session, 'response_list') and session.response_list:
            chunk_count = 0
            for chunk in session.response_list:
                if chunk.delta_content:
                    markdown_content += chunk.delta_content
                    chunk_count += 1
            logger.info(f"[生成任务 {tender_generation_id}] 提取内容完成，共 {chunk_count} 个chunk，内容长度: {len(markdown_content)} 字符")
        else:
            logger.warning(f"[生成任务 {tender_generation_id}] 会话没有response_list或为空")
        
        if not markdown_content:
            logger.error(f"[生成任务 {tender_generation_id}] 未能获取生成的内容")
            tender_generation.status = 'failed'
            db.commit()
